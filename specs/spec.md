...
- [x] 1. Daemon process model — **Two processes:** CLI and a separate child daemon. The daemon is **started/stopped by `tmrc record`** (not on login). **Bare `tmrc record` toggles** recording: if the daemon is not running, it starts it; if it is running, it stops it. Lifecycle: first `tmrc record` (or `tmrc record --start`) spawns the daemon; daemon runs until stopped by another `tmrc record` (toggle), `tmrc record --stop`, or process termination.
