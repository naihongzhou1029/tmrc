...
| 23a | Storage | Retention max age default | Load config with no `retention_max_age_days` key. | Assert the loaded `RetentionMaxAgeDays` equals `30`. | ✓ |
| 23b | Storage | Retention max disk default | Load config with no `retention_max_disk_bytes` key. | Assert the loaded `RetentionMaxDiskBytes` equals `53687091200` (50 GB). | ✓ |
| 23c | Storage | Retention max age override | Set `retention_max_age_days: 7` in config and load. | Assert the loaded `RetentionMaxAgeDays` equals `7`. | ✓ |
| 23d | Storage | Retention max disk override | Set `retention_max_disk_bytes: 10737418240` (10 GB) in config and load. | Assert the loaded `RetentionMaxDiskBytes` equals `10737418240`. | ✓ |
| 23e | Storage | Retention disable age | Set `retention_max_age_days: 0` in config and load. | Assert the loaded `RetentionMaxAgeDays` is `0`; running eviction does not delete any segments by age. | ✓ |
| 23f | Storage | Retention disable disk | Set `retention_max_disk_bytes: 0` in config and load. | Assert the loaded `RetentionMaxDiskBytes` is `0`; running eviction does not delete any segments by disk quota. | ✓ |
| 23g | Storage | Status shows retention config | Start daemon (or run status without daemon). Run `tmrc status`. | Assert output includes `retention_max_age_days` value and `retention_max_disk_bytes` value (or human-readable equivalent). | ✓ |
...
| 55a | CLI & Daemon | Daemon start via dotnet run | Run `dotnet run --project src/Tmrc.Cli/Tmrc.Cli.csproj -- record`, then `dotnet run --project src/Tmrc.Cli/Tmrc.Cli.csproj -- status`. | Assert `record` exits 0 only when daemon startup succeeds; `status` shows `Recording: yes` and a live PID. | ✓ |
